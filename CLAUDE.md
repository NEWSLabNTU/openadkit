...
- JP62 images must be built on **native arm64**, never via x86→arm64 QEMU emulation (QEMU hits intermittent `find_library` failures in cmake subprocess calls). Native arm64 includes both a real Jetson and GitHub-hosted `ubuntu-22.04-arm` runners — the latter build JP62 fine in CI (`.github/workflows/build-jp62-images.yaml`): compiling CUDA needs only the toolkit, not a GPU, so the absence of Jetson hardware is not a blocker. That workflow pins Autoware to the `1.7.1` tag because `Dockerfile.jp62` COPYs the root `*.env` files, which Autoware main has since removed
